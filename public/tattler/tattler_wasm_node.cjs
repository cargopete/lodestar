 ...
 * @returns {string}
 */
function issue_receipt(body_json, rows_json, key_hex) {
    let deferred4_0;
    let deferred4_1;
    try {
...
exports.issue_receipt = issue_receipt;
 ...
 * failure cases are as important as the success and a thrown exception encourages callers to
 * treat "malformed" and "forged" as the same thing. They are not: one is a bad paste, the other
 * is somebody lying.
 * @param {string} json
 * @returns {string}
 */
function verify_receipt(json) {
    let deferred2_0;
    let deferred2_1;
    try {
...
const wasmPath = `${__dirname}/tattler_wasm_bg.wasm`;
